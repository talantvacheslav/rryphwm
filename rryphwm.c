 ...
 *
 * The event handlers of rryphwm are organized in an array which is accessed
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "rryphwm", 8);
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
		...
		die("rryphwm: execvp '%s' failed:", ((char **)arg->v)[0]);
	}
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "rryphwm: fatal error: request code=%d, error code=%d\n",
...
{
	die("rryphwm: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (argc == 2 && !strcmp("-ignoreautostart", argv[1]))
		...
		die("usage: rryphwm [-v] [-ignoreautostart]"
          );
	if (!setlocale(LC_CTYPE, "") || !XSupportsLocale())
		fputs("warning: no locale support\n", stderr);
	if (!(dpy = XOpenDisplay(NULL)))
		die("rryphwm: cannot open display");
	checkotherwm();
	XrmInitialize();
	loadxrdb();
	setup();
